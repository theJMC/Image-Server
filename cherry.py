...
class imageServer(object):
    ...
    def index(self, folder="/"):
        ...
        start_table = "<table class='table table-showhide' id='{}'><thread><tr><th scope='col'><h1>{}</h1></th></tr></thread><tbody>"

        results = []    
        
        for item in folders:
            results.append(start_table.format(item, item.replace("^", " ")))
            os.chdir("image/" + item)
            images = os.listdir()
            os.chdir("../..")
            for photo in images:
                results.append("<tr><td><img src=/download/" + item + "/"+ photo + " class='img-thumbnail image_thumb' style=''></td></tr>")
            results.append("</tbody></table>")

        showhide_button = "<button type='button' class='btn btn-info showtable'>Show Photos</button>" + "<button type='button' class='btn btn-info hidetable'>Hide Photos</button>"
        return str(page).format(''.join(result), showhide_button, "<div class='container'> " + ''.join(results) + "</div>")
...
if __name__ == "__main__":
    config = {'/download':{
        ...
        'tools.staticdir.on': True,
        'tools.staticdir.dir': "/media/James/Image_Server_V2/icons"
    }, '/bootstrap':{
        'tools.staticdir.on': True,
        'tools.staticdir.dir': "/media/James/Image_Server_V2/node_modules/bootstrap"
    }
    }
